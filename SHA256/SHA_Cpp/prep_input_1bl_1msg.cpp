/**
 * Input data preparing for SHA256 algorithm with fixed messages of 1 block of 512 bits
 */
#include <bitset>
#include <iostream>
#include <iomanip>
...
#define C_NUM_LINES  16  // 512 bits - 16 elements of 32 bits
#define C_NUM_COLS  1024   // Change this
#define C_NUM_CELLS  128
#define C_NUM_BLOCKS C_NUM_COLS/C_NUM_CELLS
#define C_BLOCK_MESG_LENGTH C_NUM_BLOCKS * 128
...
int main()
{
    ...
    fstream new_file;
    new_file.open("text/text_1024cols.txt", ios::in);
    if (new_file.is_open()) {
    ...
    int char_pos = 0;
    unsigned int data_matrix[C_NUM_LINES][C_NUM_COLS];
...
//        cout<< final_length[i]<<endl;
    ...
    int column_width = 15;
    ofstream myfile;
    myfile.open("data/axi_stream_input_128_2_2_sha_256_1bl_fixed_1024bl_128_cells.txt");
    ...
    for ( int bl = 0; bl < C_NUM_COLS/C_NUM_CELLS; bl ++) {
        ...
        for (int line = 0; line < C_NUM_LINES; line++) {
            for (int column = 0; column < C_NUM_CELLS; column++) {
                myfile <<  setw(column_width) << data_matrix[line][column + C_NUM_CELLS * bl] << "\t";
            }
            myfile << "\n";
        }
