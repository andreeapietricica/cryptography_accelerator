/**
 * Source: https://github.com/SergeyBel/AES/blob/master/src/
 */
...
unsigned char *AES::CustomGetRoundKeys(const unsigned char key[])
{
    unsigned char *roundKeys = new unsigned char[4 * Nb * (Nr + 1)];
    KeyExpansion(key, roundKeys);
    return roundKeys;
...
unsigned char *AES::CustomOnlyEncryptECB(const unsigned char in[], unsigned int inLen,
...
unsigned char *AES::EncryptCustomCTR(const unsigned char in[], unsigned int inLen,
                               const unsigned char key[]) {
    CheckLength(inLen);
    unsigned char *out = new unsigned char[inLen];
//    unsigned char *counter = new unsigned char[inLen];
    static unsigned char counter[C_DATA_SIZE_IN_BYTES];
    unsigned int ctrTemp = 0;

    for (unsigned int j = 0; j < blockBytesLen - 4; j ++)
    {
        counter[j] = char(0);
    }
    for (unsigned int i = 0; i < inLen; i += blockBytesLen)
    {
            counter[i + blockBytesLen - 1] = char(ctrTemp & 0xFF);
            counter[i + blockBytesLen - 2] = char((ctrTemp >> 8) & 0xFF);
            counter[i + blockBytesLen - 3] = char((ctrTemp >> 16) & 0xFF);
            counter[i + blockBytesLen - 4] = char((ctrTemp >> 24) & 0xFF);

//        std::cout<< inLen<<" ";
//        std::cout<< char(ctrTemp >> 8 & 0xFF)<<" ";
            ctrTemp ++;
        for (unsigned int j = 0; j < blockBytesLen; j ++)
        {
    ...
    KeyExpansion(key, roundKeys);
    for (unsigned int i = 0; i < inLen; i += blockBytesLen) {
        EncryptBlock(counter + i, out + i, roundKeys);
    }
...
unsigned char *AES::CustomOnlyEncryptCTR(const unsigned char in[], unsigned int inLen, unsigned char roundKeys[]) {
    CheckLength(inLen);
    unsigned char *out = new unsigned char[inLen];
//    unsigned char *counter = new unsigned char[inLen];
    static unsigned char counter[C_DATA_SIZE_IN_BYTES];
